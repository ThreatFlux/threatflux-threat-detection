...
use crate::{
    CacheConfig, CacheEntry, CacheError, Result,
    StorageBackend, EntryMetadata,
    eviction::{EvictionStrategy, EvictionContext},
    search::Searchable,
};
...
{
    entries: Arc<RwLock<HashMap<K, Vec<CacheEntry<K, V, M>>>>>,
    config: CacheConfig,
    backend: Arc<B>,
    ...
    B: StorageBackend<Key = K, Value = V, Metadata = M>,
{
    ...
    async fn remove(&self, key: &K) -> std::result::Result<Option<V>, Self::Error> {
        ...
        Ok(removed.and_then(|entries| entries.into_iter().last().map(|e| e.value)))
    }
