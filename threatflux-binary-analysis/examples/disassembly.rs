...
use threatflux_binary_analysis::{
    ...
    types::{InstructionCategory, ControlFlow},
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for section in binary.sections() {
        if section.permissions.execute {
            println!("\nDisassembling section: {} (0x{:x})", section.name, section.address);
    ...
    if let Some(entry_point) = binary.entry_point() {
        ...
        for section in binary.sections() {
            if entry_point >= section.address && 
               entry_point < section.address + section.size {
...
fn analyze_instruction_stats(instructions: &[threatflux_binary_analysis::types::Instruction]) {
    ...
    let jumps: Vec<_> = instructions.iter()
        .filter(|i| matches!(i.flow, ControlFlow::Jump(_) | ControlFlow::ConditionalJump(_)))
        .collect();
    
    let calls: Vec<_> = instructions.iter()
