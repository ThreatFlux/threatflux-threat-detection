...
use threatflux_binary_analysis::{
    BinaryFile,
    analysis::control_flow::{ControlFlowAnalyzer, AnalysisConfig},
    types::Architecture,
};
    ...
    match analyzer.analyze_binary(&binary) {
        Ok(cfgs) => {
            ...
            for (i, cfg) in cfgs.iter().enumerate().take(10) {
                ...
                println!("  Address range: 0x{:x} - 0x{:x}", 
                         cfg.function.start_address, 
                         cfg.function.end_address);
                ...
                for (j, block) in cfg.basic_blocks.iter().enumerate().take(5) {
                    println!("    Block {}: 0x{:x} - 0x{:x} ({} instructions)",
                             j,
                             block.start_address,
                             block.end_address,
            ...
            let total_complexity: u32 = cfgs.iter().map(|cfg| cfg.complexity.cyclomatic_complexity).sum();
