...
fn test_suspicious_detection() {
    ...
    assert_eq!(stats.total_unique_strings, 1);
    assert!(stats.suspicious_strings.contains(&"http://malware.com/payload".to_string()));
}
...
fn test_categorization() {
    ...
    for (string, expected_category) in test_cases {
        tracker.track_strings_from_results(
            &[string.to_string()],
            "/test/file",
            "hash123",
            "test_tool"
        ...
        assert!(details.categories.iter().any(|c| c.contains(expected_category)),
            "String '{}' should have category '{}'", string, expected_category);
    }
}