...
fn test_npm_package_analysis() {
    ...
    assert!(analysis.dependencies.vulnerability_summary.vulnerable_packages.contains(&"event-stream".to_string()));
    assert!(analysis.dependencies.vulnerability_summary.vulnerable_packages.contains(&"lodash".to_string()));
    ...
    let suspicious_urls: Vec<&str> = analysis.security_analysis.network_access_patterns
        .iter()
        .filter(|p| p.is_suspicious)
        .map(|p| p.url.as_str())
        .collect();
    assert!(suspicious_urls.iter().any(|url| url.contains("suspicious-site.com")));
    
    println!("NPM Analysis Test Results:");
    println!("Package: {} v{}", analysis.package_info.name, analysis.package_info.version);
    println!("Vulnerabilities found: {}", analysis.dependencies.vulnerability_summary.total_count);
    println!("Risk score: {:.1}/100", analysis.malicious_indicators.overall_risk_score);
