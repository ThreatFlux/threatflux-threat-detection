...
use file_scanner::control_flow::{
    BasicBlock, BlockType, CfgEdge, ControlFlowAnalysis, ControlFlowGraph, ControlFlowMetrics,
    EdgeType, FlowControl, Instruction, InstructionType, Loop, LoopType, OverallMetrics,
    AnalysisStats,
};
...
fn create_simple_cfg(
    ...
    let entry_instructions = vec![
        create_instruction(function_address, "push", "rbp", InstructionType::Memory, FlowControl::Fall),
        create_instruction(function_address + 1, "mov", "rbp, rsp", InstructionType::Memory, FlowControl::Fall),
        create_instruction(function_address + 2, "add", "rax, rbx", InstructionType::Arithmetic, FlowControl::Fall),
    ];
    blocks.push(create_basic_block(0, function_address, entry_instructions, BlockType::Entry));
    
    // Main block with control flow
    let main_instructions = vec![
        create_instruction(function_address + 10, "cmp", "rax, 0", InstructionType::Conditional, FlowControl::Fall),
        create_instruction(function_address + 11, "je", "0x1100", InstructionType::Conditional, FlowControl::Branch(0x1100)),
    ];
    blocks.push(create_basic_block(1, function_address + 10, main_instructions, BlockType::Conditional));
    
    // Exit block
    let exit_instructions = vec![
        create_instruction(function_address + 20, "pop", "rbp", InstructionType::Memory, FlowControl::Fall),
        create_instruction(function_address + 21, "ret", "", InstructionType::Return, FlowControl::Return),
    ];
    blocks.push(create_basic_block(2, function_address + 20, exit_instructions, BlockType::Exit));
    ...
    ControlFlowGraph {
        ...
        basic_blocks: blocks,
        edges: vec![
            CfgEdge { from_block: 0, to_block: 1, edge_type: EdgeType::Fall },
            CfgEdge { from_block: 1, to_block: 2, edge_type: EdgeType::Branch },
        ],
        entry_block: 0,
        exit_blocks: vec![2],
        loops: vec![],
...
fn create_complex_cfg(
    ...
    let entry_instructions = vec![
        create_instruction(function_address, "push", "rbp", InstructionType::Memory, FlowControl::Fall),
        create_instruction(function_address + 1, "mov", "rbp, rsp", InstructionType::Memory, FlowControl::Fall),
        create_instruction(function_address + 2, "mov", "rcx, 10", InstructionType::Memory, FlowControl::Fall),
    ];
    blocks.push(create_basic_block(0, function_address, entry_instructions, BlockType::Entry));
    
    // Loop header
    let loop_header_instructions = vec![
        create_instruction(function_address + 10, "cmp", "rcx, 0", InstructionType::Conditional, FlowControl::Fall),
        create_instruction(function_address + 11, "jle", "0x1200", InstructionType::Conditional, FlowControl::Branch(0x1200)),
    ];
    blocks.push(create_basic_block(1, function_address + 10, loop_header_instructions, BlockType::LoopHeader));
    
    // Loop body with nested complexity
    let loop_body_instructions = vec![
        create_instruction(function_address + 20, "call", "process_item", InstructionType::Call, FlowControl::Call(0x2000)),
        create_instruction(function_address + 21, "dec", "rcx", InstructionType::Arithmetic, FlowControl::Fall),
        create_instruction(function_address + 22, "cmp", "rax, 0", InstructionType::Conditional, FlowControl::Fall),
        create_instruction(function_address + 23, "je", "0x1100", InstructionType::Conditional, FlowControl::Branch(0x1100)),
        create_instruction(function_address + 24, "mul", "rax, rbx", InstructionType::Arithmetic, FlowControl::Fall),
        create_instruction(function_address + 25, "div", "rax, rdx", InstructionType::Arithmetic, FlowControl::Fall),
    ];
    blocks.push(create_basic_block(2, function_address + 20, loop_body_instructions, BlockType::LoopBody));
    
    // Exit block
    let exit_instructions = vec![
        create_instruction(function_address + 30, "xor", "rax, rax", InstructionType::Logic, FlowControl::Fall),
        create_instruction(function_address + 31, "pop", "rbp", InstructionType::Memory, FlowControl::Fall),
        create_instruction(function_address + 32, "ret", "", InstructionType::Return, FlowControl::Return),
    ];
    blocks.push(create_basic_block(3, function_address + 30, exit_instructions, BlockType::Exit));
    ...
    ControlFlowGraph {
        ...
        edges: vec![
            CfgEdge { from_block: 0, to_block: 1, edge_type: EdgeType::Fall },
            CfgEdge { from_block: 1, to_block: 2, edge_type: EdgeType::Branch },
            CfgEdge { from_block: 1, to_block: 3, edge_type: EdgeType::Branch },
            CfgEdge { from_block: 2, to_block: 1, edge_type: EdgeType::Jump }, // Loop back
        ],
        entry_block: 0,
        exit_blocks: vec![3],
        loops: vec![Loop {
...
fn create_control_flow_analysis(cfgs: Vec<ControlFlowGraph>) -> ControlFlowAnalysis {
    ...
    let average_complexity = if total_functions > 0 {
        cfgs.iter().map(|cfg| cfg.complexity.cyclomatic_complexity as f64).sum::<f64>() / total_functions as f64
    } else {
        0.0
    };
    let (max_complexity, function_with_max_complexity) = cfgs.iter()
...
fn test_halstead_metrics() {
    ...
    assert_eq!(metrics.halstead_metrics.vocabulary, 
               metrics.halstead_metrics.distinct_operators + metrics.halstead_metrics.distinct_operands);
    assert_eq!(metrics.halstead_metrics.length, 
...
fn test_parameter_count_estimation() {
    ...
    let instructions = vec![
        create_instruction(0x1000, "push", "rbp", InstructionType::Memory, FlowControl::Fall),
        create_instruction(0x1001, "mov", "rbp, rsp", InstructionType::Memory, FlowControl::Fall),
        create_instruction(0x1002, "mov", "rax, rdi", InstructionType::Memory, FlowControl::Fall),
        create_instruction(0x1003, "add", "rax, rsi", InstructionType::Arithmetic, FlowControl::Fall),
        create_instruction(0x1004, "mov", "rbx, rdx", InstructionType::Memory, FlowControl::Fall),
    ];
    
    let cfg = ControlFlowGraph {
        function_address: 0x1000,
        function_name: "param_func".to_string(),
        basic_blocks: vec![create_basic_block(0, 0x1000, instructions, BlockType::Entry)],
        edges: vec![],
        entry_block: 0,
        exit_blocks: vec![0],
        loops: vec![],
...
fn test_overall_metrics_calculation() {
    ...
    assert_eq!(analysis.overall_metrics.most_complex_function, Some("func2".to_string()));
...
fn test_quality_report_generation() {
        ...
        basic_blocks: {
            ...
            for i in 0..150 {
                instructions.push(create_instruction(
                    ...
                    "rax, rbx",
                    if i % 10 == 0 { InstructionType::Conditional } else { InstructionType::Memory },
                    FlowControl::Fall,
                ));
            }
            blocks.push(create_basic_block(0, 0x1000, instructions, BlockType::Normal));
            blocks
        },
        edges: vec![],
        entry_block: 0,
    ...
    let has_complexity_issue = analysis.quality_report.quality_issues.iter()
        .any(|i| matches!(i.issue_type, QualityIssueType::HighComplexity));
    assert!(has_complexity_issue);
    
    let has_length_issue = analysis.quality_report.quality_issues.iter()
        .any(|i| matches!(i.issue_type, QualityIssueType::LongFunction));
    assert!(has_length_issue);
    
    let has_nesting_issue = analysis.quality_report.quality_issues.iter()
    ...
    // Check code health
    assert!(matches!(analysis.quality_report.code_health, CodeHealth::Critical | CodeHealth::Poor));
}
...
fn test_code_health_classification() {
    ...
    assert!(matches!(analysis.quality_report.code_health, CodeHealth::Excellent | CodeHealth::Good));
}
...
fn test_multiple_return_paths() {
    ...
    let has_return_issue = analysis.quality_report.quality_issues.iter()
...
fn test_god_function_detection() {
    ...
    for i in 0..120 {
        instructions.push(create_instruction(
            ...
            "rax, rbx",
            if i % 5 == 0 { InstructionType::Conditional } else { InstructionType::Memory },
            FlowControl::Fall,
        ));
    }
    ...
    let cfg = ControlFlowGraph {
        ...
        basic_blocks: vec![create_basic_block(0, 0x1000, instructions, BlockType::Normal)],
        edges: vec![],
        entry_block: 0,
        exit_blocks: vec![0],
        loops: vec![],
    ...
    let has_god_function_issue = analysis.quality_report.quality_issues.iter()
...
fn test_serialization_deserialization() {
    ...
    assert_eq!(metrics.halstead_metrics.volume, deserialized.halstead_metrics.volume);
}
...
fn test_recommendations() {
    ...
    assert!(!analysis.quality_report.recommendations.is_empty());
    let has_complexity_recommendation = analysis.quality_report.recommendations.iter()
