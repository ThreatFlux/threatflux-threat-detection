...
fn test_multiple_occurrences_same_string() {
    ...
    for i in 0..3 {
        tracker.track_string(
            ...
            "same_hash",
            "scanner",
            StringContext::FileString { offset: Some(i * 50) },
        ).unwrap();
    }
...
fn test_string_categorization_via_tracking() {
    ...
    tracker.track_strings_from_results(&strings, "/test/file", "test_hash", "test_tool").unwrap();
...
fn test_suspicious_string_detection() {
    ...
    for string in &suspicious_strings {
        ...
        let entry = tracker.get_string_details(string).unwrap();
        assert!(entry.is_suspicious, "String '{}' should be marked as suspicious", string);
    }
    ...
    for string in &normal_strings {
        ...
        let entry = tracker.get_string_details(string).unwrap();
        assert!(!entry.is_suspicious, "String '{}' should not be marked as suspicious", string);
    }
}
...
fn test_entropy_based_suspicion() {
    ...
    let test_cases = vec![
        ...
        ("SGVsbG8gV29ybGQhIFRoaXMgaXMgYSBiYXNlNjQgZW5jb2RlZCBzdHJpbmc=", true), // Base64 - suspicious
    ];
    
    for (string, should_be_suspicious) in test_cases {
        tracker.track_string(string, "/test/file", "hash", "scanner",
            StringContext::FileString { offset: None }).unwrap();
        
        let entry = tracker.get_string_details(string).unwrap();
        assert_eq!(
            entry.is_suspicious, should_be_suspicious,
            "String '{}' suspicious flag mismatch", string
        );
    }
}
...
fn test_string_filtering() {
    ...
    let strings = vec![
        ...
        ("this_is_a_very_long_string_for_testing_length_filters", 3, false),
...
fn test_related_strings() {
    ...
    tracker.track_string("kernel32.dll", "/malware.exe", "hash1", "imports", 
        StringContext::Import { library: "kernel32.dll".to_string() }).unwrap();
    tracker.track_string("ntdll.dll", "/malware.exe", "hash1", "imports",
        StringContext::Import { library: "ntdll.dll".to_string() }).unwrap();
    tracker.track_string("advapi32.dll", "/malware.exe", "hash1", "imports",
        StringContext::Import { library: "advapi32.dll".to_string() }).unwrap();
    
    // Group 2: Different file, different category
    tracker.track_string("unrelated_string", "/other.txt", "hash2", "strings",
        StringContext::FileString { offset: None }).unwrap();
...
fn test_statistics_generation() {
    ...
    tracker.track_string("https://malware.com", "/mal.exe", "mal_hash", "scanner",
        StringContext::Url { protocol: Some("https".to_string()) }).unwrap();
    tracker.track_string("cmd.exe", "/mal.exe", "mal_hash", "scanner",
        StringContext::Command { command_type: "shell".to_string() }).unwrap();
...
fn test_track_strings_from_results() {
    ...
    let url_entry = tracker.get_string_details("https://example.com/api").unwrap();
    ...
    let registry_entry = tracker.get_string_details("HKEY_LOCAL_MACHINE\\SOFTWARE\\App").unwrap();
    assert!(registry_entry.categories.contains("registry"));
    
    let command_entry = tracker.get_string_details("powershell.exe -Command").unwrap();
...
fn test_occurrence_limit() {
    ...
    let entry = tracker.get_string_details("frequently_occurring_string").unwrap();
...
fn test_category_filtering() {
    ...
    tracker.track_string("https://example.com", "/file1", "hash1", "scanner",
        StringContext::Url { protocol: Some("https".to_string()) }).unwrap();
    tracker.track_string("/usr/bin/test", "/file2", "hash2", "scanner",
        StringContext::Path { path_type: "system".to_string() }).unwrap();
    tracker.track_string("kernel32.dll", "/file3", "hash3", "scanner",
        StringContext::Import { library: "kernel32.dll".to_string() }).unwrap();
    tracker.track_string("normal string", "/file4", "hash4", "scanner",
        StringContext::FileString { offset: None }).unwrap();
...
fn test_file_hash_filtering() {
    ...
    tracker.track_string("string1", "/file1", "hash_a", "scanner",
        StringContext::FileString { offset: None }).unwrap();
    tracker.track_string("string2", "/file2", "hash_b", "scanner",
        StringContext::FileString { offset: None }).unwrap();
    tracker.track_string("string3", "/file3", "hash_c", "scanner",
        StringContext::FileString { offset: None }).unwrap();
    
    // Also add string1 from another file
    tracker.track_string("string1", "/file4", "hash_d", "scanner",
        StringContext::FileString { offset: None }).unwrap();
...
fn test_serialization_roundtrip() {
    ...
    let entry = StringEntry {
        ...
        occurrences: vec![
            StringOccurrence {
                ...
                timestamp: Utc::now(),
                context: StringContext::Import { library: "test.dll".to_string() },
            }
        ],
    ...
    assert_eq!(deserialized.high_entropy_strings.len(), stats.high_entropy_strings.len());
}
...
fn test_edge_cases() {
    ...
    // Empty string
    tracker.track_string("", "/file", "hash", "scanner",
        StringContext::FileString { offset: None }).unwrap();
    ...
    tracker.track_string(&long_string, "/file", "hash", "scanner",
        StringContext::FileString { offset: None }).unwrap();
    ...
    tracker.track_string(control_string, "/file", "hash", "scanner",
        StringContext::FileString { offset: None }).unwrap();
    ...
    tracker.track_string(unicode_string, "/file", "hash", "scanner",
        StringContext::FileString { offset: None }).unwrap();
...
fn test_similarity_calculation() {
    ...
    for s in &file1_strings {
        tracker.track_string(s, "/malware1.exe", "hash1", "imports",
            StringContext::Import { library: "kernel32.dll".to_string() }).unwrap();
    }
    ...
    for s in &file2_strings {
        tracker.track_string(s, "/program2.exe", "hash2", "imports",
            StringContext::Import { library: "libc.so".to_string() }).unwrap();
    }
