...
fn detect_anti_analysis(
    ...
    if let Some(strings) = strings {
        for (indicator, desc) in &debug_indicators {
            if strings.ascii_strings.iter().any(|s| s.contains(indicator)) ||
               strings.unicode_strings.iter().any(|s| s.contains(indicator)) {
    ...
    if let Some(disassembly) = disassembly {
        ...
        for pattern in suspicious_patterns {
            if let Some(desc) = obfuscation_indicators.iter()
...
fn detect_persistence_mechanisms(
    ...
    let registry_persistence = vec![
        ("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run", PersistenceType::RegistryKeys, "Startup registry key"),
        ("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\RunOnce", PersistenceType::RegistryKeys, "Run once registry key"),
        ("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\RunServices", PersistenceType::RegistryKeys, "Run services key"),
        ("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Shell Folders", PersistenceType::RegistryKeys, "Shell folders manipulation"),
        ("SOFTWARE\\Classes\\", PersistenceType::RegistryKeys, "File association hijacking"),
        ("SYSTEM\\CurrentControlSet\\Services", PersistenceType::ServiceInstallation, "Service installation"),
        ("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon", PersistenceType::RegistryKeys, "Winlogon manipulation"),
        ("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies", PersistenceType::RegistryKeys, "Policy modification"),
    ];
...
fn detect_network_patterns(
    ...
    if let Some(strings) = strings {
        ...
        for s in &strings.ascii_strings {
            if domain_pattern.is_match(s) && s.contains('.') && !s.ends_with(".dll") && !s.ends_with(".exe") {
                domain_count += 1;
                if domain_count >= 3 {
...
fn detect_file_operations(
    ...
    for (op_type, apis) in found_ops {
        let suspicious = matches!(op_type, 
...
fn detect_registry_operations(strings: Option<&ExtractedStrings>) -> Vec<RegistryOperation> {
    ...
    if let Some(strings) = strings {
        ...
        let registry_apis = vec![
            ("RegCreateKey", RegistryOpType::KeyCreation, "Registry key creation"),
            ("RegOpenKey", RegistryOpType::ValueModification, "Registry key access"),
            ("RegSetValue", RegistryOpType::ValueModification, "Registry value setting"),
            ("RegDeleteKey", RegistryOpType::KeyDeletion, "Registry key deletion"),
            ("RegDeleteValue", RegistryOpType::KeyDeletion, "Registry value deletion"),
            ("RegQueryValue", RegistryOpType::ValueModification, "Registry value query"),
        ];
...
fn detect_process_operations(
    ...
    let process_apis = vec![
        ("CreateProcess", ProcessOpType::ProcessCreation, vec!["Process creation"]),
        ("CreateThread", ProcessOpType::ThreadCreation, vec!["Thread creation"]),
        ("CreateRemoteThread", ProcessOpType::ProcessInjection, vec!["Remote thread injection"]),
        ("OpenProcess", ProcessOpType::ProcessInjection, vec!["Process handle acquisition"]),
        ("TerminateProcess", ProcessOpType::ProcessTermination, vec!["Process termination"]),
        ("WriteProcessMemory", ProcessOpType::ProcessInjection, vec!["Memory writing"]),
        ("ReadProcessMemory", ProcessOpType::ProcessInjection, vec!["Memory reading"]),
        ("VirtualAllocEx", ProcessOpType::ProcessInjection, vec!["Remote memory allocation"]),
        ("SetThreadContext", ProcessOpType::ProcessInjection, vec!["Thread context manipulation"]),
        ("QueueUserAPC", ProcessOpType::ProcessInjection, vec!["APC injection"]),
        ("NtCreateThreadEx", ProcessOpType::ProcessInjection, vec!["Native thread creation"]),
        ("RtlCreateUserThread", ProcessOpType::ProcessInjection, vec!["User thread creation"]),
    ];
    ...
    if let Some(symbols) = symbols {
        for (api, op_type, techniques) in &process_apis {
            if symbols.functions.iter().any(|f| f.name.contains(api)) {
                found_by_type.entry(op_type.clone())
                    .or_insert_with(Vec::new)
...
fn identify_suspicious_behaviors(
    ...
    let has_kernel_apis = process_ops.iter()
        .any(|op| op.techniques.iter().any(|t| t.contains("Nt") || t.contains("Zw")));
    let has_process_hiding = process_ops.iter()
        .any(|op| matches!(op.operation_type, ProcessOpType::ProcessInjection));
    let has_service_install = persistence.iter()
    ...
    let has_file_access = file_ops.iter()
        .any(|op| matches!(op.operation_type, FileOpType::FileCopying));
    let has_credential_apis = process_ops.iter()
        .any(|op| op.techniques.iter().any(|t| 
            t.contains("Cred") || t.contains("Token") || t.contains("Password")
        ));
    ...
    let has_download = network.iter()
        .any(|n| n.indicators.iter().any(|i| 
            i.contains("URLDownloadToFile") || i.contains("InternetReadFile")
        ));
    let has_file_write = file_ops.iter()
        .any(|op| matches!(op.operation_type, FileOpType::FileCreation));
    let has_exec = process_ops.iter()
        .any(|op| matches!(op.operation_type, ProcessOpType::ProcessCreation));
...
fn generate_behavioral_recommendations(
    ...
    if evasion_score > 70.0 {
        recommendations.push("CRITICAL: Very high evasion score. Use advanced sandbox with anti-evasion bypasses.".to_string());
        recommendations.push("Consider manual reverse engineering in isolated environment.".to_string());
    } else if evasion_score > 40.0 {
        recommendations.push("High evasion techniques detected. Extended sandbox analysis recommended.".to_string());
    ...
    for technique in anti_analysis {
        match technique.technique_type {
            AntiAnalysisType::AntiDebug => {
                recommendations.push("Anti-debugging detected. Use kernel-mode debugger or bypass techniques.".to_string());
            }
            AntiAnalysisType::AntiVM => {
                recommendations.push("VM detection present. Use bare-metal analysis or VM hiding techniques.".to_string());
            }
            AntiAnalysisType::AntiSandbox => {
                recommendations.push("Sandbox evasion detected. Extend analysis time and simulate user activity.".to_string());
            }
            AntiAnalysisType::ProcessHollowing => {
                recommendations.push("Process hollowing capability. Monitor child process creation closely.".to_string());
            }
            _ => {}
        }
    }
    ...
    if !persistence.is_empty() {
        recommendations.push("Persistence mechanisms detected. Check system after reboot.".to_string());
        
        for mechanism in persistence {
            match mechanism.mechanism_type {
                PersistenceType::RegistryKeys => {
                    recommendations.push("Registry persistence found. Monitor registry changes.".to_string());
                }
                PersistenceType::ServiceInstallation => {
                    recommendations.push("Service installation capability. Check installed services.".to_string());
                }
                PersistenceType::ScheduledTasks => {
                    recommendations.push("Scheduled task persistence. Review task scheduler.".to_string());
                }
                _ => {}
            }
        }
    ...
    for pattern in network {
        match pattern.pattern_type {
            NetworkPatternType::CommandAndControl => {
                recommendations.push("C&C communication detected. Monitor network traffic closely.".to_string());
                recommendations.push("Block identified C&C servers at firewall.".to_string());
            }
            NetworkPatternType::TorUsage => {
                recommendations.push("Tor usage detected. Sample may use hidden services.".to_string());
            }
            NetworkPatternType::DataExfiltration => {
                recommendations.push("Data exfiltration capability. Monitor outbound connections.".to_string());
            }
            _ => {}
        }
    }
    ...
    for behavior in suspicious_behaviors {
        match behavior.behavior_type.as_str() {
            "Ransomware" => {
                recommendations.push("RANSOMWARE BEHAVIOR DETECTED! Isolate immediately.".to_string());
            ...
            "Rootkit" => {
                recommendations.push("Rootkit behavior detected. Use rootkit detection tools.".to_string());
                recommendations.push("Check for hidden processes and files.".to_string());
            }
            "Backdoor" => {
                recommendations.push("Backdoor functionality detected. Scan for network listeners.".to_string());
                recommendations.push("Check all persistence locations.".to_string());
            }
            "DataTheft" => {
                recommendations.push("Data theft capability. Monitor sensitive file access.".to_string());
