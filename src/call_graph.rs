...
pub fn generate_call_graph(
    ...
    for instruction in &disassembly.instructions {
        ...
        if let Some(func) = symbols.functions.iter().find(|f| 
            instruction.address >= f.address && 
            instruction.address < f.address + f.size
        ) {
        ...
        match &instruction.flow_control {
            ...
            Some(FlowControl::Jump { target, conditional }) => {
                ...
                if let (Some(caller), Some(target_addr), false) = (current_function, target, conditional) {
                    if let Some(func) = symbols.functions.iter().find(|f| 
                        *target_addr == f.address
                    ) {
...
impl CallGraph {
    ...
    fn find_unreachable_functions(&mut self) {
        ...
        // Find unreachable functions
        self.unreachable_functions = self.nodes.iter()
        ...
        while let Some((current, depth)) = queue.pop_front() {
            for edge in &self.edges {
                if edge.caller == current {
                    ...
                    let should_update = depths.get(&edge.callee)
                        .map(|&d| new_depth < d)
                        .unwrap_or(true);
    ...
    fn calculate_statistics(&mut self) {
        ...
        let recursive_functions = self.nodes.iter()
            .filter(|n| n.is_recursive)
            .count();
        
        let leaf_functions = self.nodes.iter()
            .filter(|n| n.out_degree == 0)
            .count();
        
        let root_functions = self.nodes.iter()
            .filter(|n| n.in_degree == 0)
            .count();
        
        let total_in_degree: u32 = self.nodes.iter()
            .map(|n| n.in_degree)
            .sum();
        
        let total_out_degree: u32 = self.nodes.iter()
            .map(|n| n.out_degree)
            .sum();
