...
mod tests {
    ...
    fn test_calculate_blake3() {
        ...
        assert_eq!(hash, "288a86a79f20a3d6dccdca7713beaed178798296bdfa7913fa2a62d9727bf8f8");
    }
    ...
    async fn test_calculate_all_hashes() {
        ...
        assert_eq!(hashes.sha256, "6c76f7bd4b84eb68c26d2e8f48ea76f90b9bdf8836e27235a0ca4325f8fe4ce5");
    ...
    async fn test_empty_file_hashes() {
        ...
        assert_eq!(hashes.blake3, "af1349b9f5f9a1a6a0404dea36dcc9499bcb25c9adc112b7cc9a93cae41f3262");
    }
    ...
    async fn test_concurrent_hash_calculation() {
        ...
        for _ in 0..10 {
            ...
            let handle = tokio::spawn(async move {
                calculate_all_hashes(&path).await
            });
            handles.push(handle);
        }
    ...
    fn test_hash_format() {
        ...
        assert!(md5.chars().all(|c| c.is_ascii_hexdigit() && !c.is_uppercase()));
        assert!(sha256.chars().all(|c| c.is_ascii_hexdigit() && !c.is_uppercase()));
        assert!(sha512.chars().all(|c| c.is_ascii_hexdigit() && !c.is_uppercase()));
        assert!(blake3.chars().all(|c| c.is_ascii_hexdigit() && !c.is_uppercase()));
