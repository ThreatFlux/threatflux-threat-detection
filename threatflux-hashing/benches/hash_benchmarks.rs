...
use threatflux_hashing::{calculate_all_hashes, calculate_all_hashes_with_config, HashConfig, HashAlgorithms};
use std::fs::File;
use std::io::Write;
use std::path::Path;
...
fn bench_individual_algorithms(c: &mut Criterion) {
    ...
    group.bench_function("md5_only", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                calculate_all_hashes_with_config(black_box(&file_path), &config).await.unwrap()
            })
        });
    });
...
criterion_group!(benches, bench_all_hashes, bench_individual_algorithms, bench_buffer_sizes);
